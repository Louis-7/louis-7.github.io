...
const loadingAnimalsCanvas = document.getElementById(
  'loading-animals-canvas'
...
let settingsPanel: HTMLElement | null,
  seasonBtn: HTMLElement | null,
  editorToggleBtn: HTMLElement | null,
  debugToggleBtn: HTMLElement | null,
...
const ZONES: any[] = [
  ...
  },
  {
    ...
    y: 0,
    w: 150,
    h: 150, // Resized to match 30px grid (150/30 = 5)
    type: 'garden',
  ...
  },
  {
    ...
    y: -210,
    w: 30,
    h: 50, // Moved right next to house door
    type: 'mailbox',
  ...
  animalsToSpawn.forEach((type) => {
    ...
    while (!placed && attempts < 50) {
      ...
      if (attempts < 20) {
        ...
        // Anywhere
        x =
          Math.random() * (WORLD_LIMITS.maxX - WORLD_LIMITS.minX) +
          WORLD_LIMITS.minX;
        y =
          Math.random() * (WORLD_LIMITS.maxY - WORLD_LIMITS.minY) +
          WORLD_LIMITS.minY;
      }
      ...
      for (const zone of ZONES) {
        if (
          x > zone.x - buffer &&
          x < zone.x + zone.w + buffer &&
          y > zone.y - buffer &&
          y < zone.y + zone.h + buffer
        ) {
          hitZone = true;
          break;
        }
...
function initSettingsListeners() {
  ...
  debugToggleBtn!.addEventListener('click', () => {
    ...
    debugToggleBtn!.innerText = gameState.debug.showCoords
      ? 'Hide Coords'
      : 'Show Coords';
  });
}
...
canvas.addEventListener('mousedown', (e) => {
  ...
  if (gameState.editor.tool === 'erase') {
    ...
    let idx = SCENERY.findIndex(
      (s) => Math.hypot(s.x - worldX, s.y - worldY) < clickThreshold
    );
    if (idx !== -1) {
      SCENERY.splice(idx, 1);
    } else {
      // If not found, check ANIMALS
      idx = ANIMALS.findIndex(
        (a) => Math.hypot(a.x - worldX, a.y - worldY) < clickThreshold
      );
      if (idx !== -1) {
    ...
    ['cow', 'chicken', 'horse', 'sheep'].includes(gameState.editor.tool)
  ) {
    // Add animal
...
const mobileInteractionPrompt = document.getElementById(
...
const loadingAnimals: LoadingAnimal[] = [
  ...
  },
  {
    ...
    direction: 'right',
    frame: 0,
    bounceOffset: Math.PI * 1.5,
  },
];

function drawLoadingAnimal(
  ctx: CanvasRenderingContext2D,
  animal: LoadingAnimal
) {
  const x = animal.x;
  const y = animal.y;
...
function drawLoadingScreen() {
  ...
  loadingCtx.fillRect(
    0,
    0,
    loadingAnimalsCanvas.width,
    loadingAnimalsCanvas.height
  );
...
async function init() {
  ...
  const fontLoad = document.fonts.ready.catch((e) =>
    console.warn('Font load warning:', e)
  );
  const blogLoad = fetchBlogs().catch((e) =>
    console.warn('Blog load warning:', e)
  );
  ...
  ANIMALS.forEach((animal) => {
    ...
    if (animal.isMoving) {
      if (animal.moveTimer <= 0) {
        ...
        if (newX < WORLD_LIMITS.minX + 20 || newX > WORLD_LIMITS.maxX - 20)
          newX = animal.x;
        if (newY < WORLD_LIMITS.minY + 20 || newY > WORLD_LIMITS.maxY - 20)
          newY = animal.y;
...
function draw() {
  ...
  const entities = [
    ...
    ...ZONES.map((z) => ({
      ...z,
      drawType: 'zone',
      z: z.y + z.collisionBox.h + z.collisionBox.y,
    })),
...
function drawCoordinates() {
  ...
  // X Axis Ticks
  for (
    let x = Math.ceil(WORLD_LIMITS.minX / 100) * 100;
    x <= WORLD_LIMITS.maxX;
    x += 100
  ) {
    if (x === 0) continue;
  ...
  for (
    let y = Math.ceil(WORLD_LIMITS.minY / 100) * 100;
    y <= WORLD_LIMITS.maxY;
    y += 100
  ) {
    if (y === 0) continue;
...
function drawGround() {
  ...
  for (let x = startX; x < startX + w; x += 50) {
    for (let y = startY; y < startY + h; y += 50) {
      ...
      if (
        x + offsetX < WORLD_LIMITS.minX ||
        x + offsetX > WORLD_LIMITS.maxX ||
        y + offsetY < WORLD_LIMITS.minY ||
        y + offsetY > WORLD_LIMITS.maxY
      ) {
        // Draw wave foam
  ...
  ctx.fillRect(
    WORLD_LIMITS.minX,
    WORLD_LIMITS.minY,
    WORLD_LIMITS.maxX - WORLD_LIMITS.minX,
    WORLD_LIMITS.maxY - WORLD_LIMITS.minY
  );
